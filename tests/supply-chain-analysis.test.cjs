...
assert.match(html, /function supplyChainDistributionWarehouse\(row\)[\s\S]*supplyChainProvinceWarehouses\[row\.province\] \|\| regionDefault \|\| \(firstRegionWarehouse/);
...
assert.match(html, /id="supplyChainDistributionClearAllBtn"[^>]*>清除所有（包含销售数据）<\/button>[\s\S]*id="supplyChainDistributionClearSalesBtn"[^>]*>清除销售数据<\/button>[\s\S]*id="supplyChainDistributionClearAllocationBtn"[^>]*>清除分货<\/button>/);
assert.match(html, /function clearCurrentSupplyChainDistribution\(\)[\s\S]*supplyChainProvinceSnapshots\.filter[\s\S]*refreshSupplyChainDistributionSkus/);
assert.match(html, /function clearCurrentSupplyChainSales\(\)[\s\S]*supplyChainProvinceSnapshots\.filter[\s\S]*supplyChainProvinceSnapshots/);
assert.match(html, /function clearCurrentSupplyChainAllocation\(\)[\s\S]*delete supplyChainDistributionQuantities\[quantityKey\][\s\S]*delete supplyChainDistributionUnits\[quantityKey\][\s\S]*delete supplyChainDistributionFinalQuantities\[key\]/);
...
assert.match(html, /id="supply-chain-archive"[\s\S]*省份所属大区按内置固定规则划分[\s\S]*<th>区域 \/ 省份<\/th><th>配送中心<\/th>/);
...
assert.match(html, /function prepareSupplyChainArchiveOptions\(input\)[\s\S]*if \(!warehouses\.length\) warehouses = supplyChainWarehouseRegions;/);
assert.match(html, /warehouse\.region !== region && supplyChainWarehouseRegions\.some/);
...
assert.match(html, /function prepareSupplyChainArchiveOptions\(input\)[\s\S]*JDSupplyChain\.provinceRegion\(province\)[\s\S]*item\.region === region/);
...
assert.match(html, /隐藏不在 SKU 档案中的 SKU[\s\S]*id="supplyChainHideSkuToggle"[\s\S]*隐藏无实际内配数量的 SKU[\s\S]*id="supplyChainHideNoActualSkuToggle"/);
